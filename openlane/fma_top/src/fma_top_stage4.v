// =============================================================================
// Stage 4: Complete FMA Top Module
// Supports: mode 000=FP32, 001=FP16, 010=FP8x4,
//           011=INT8 MAD, 100=FP16->FP32 Acc, 101=FP8x4->FP32 Acc
// Incremental on Stage 3: adds INT8 multiply-add (MAD)
// =============================================================================

module fma_top_stage4 (
    input  wire        clk,
    input  wire        rst_n,
    input  wire        valid_in,
    input  wire [2:0]  mode,
    ...
    wire is_int8  = (mode == 3'b011);
    wire is_acc16 = (mode == 3'b100);
    wire is_acc8  = (mode == 3'b101);
    wire is_acc   = is_acc16 | is_acc8;

    reg [31:0] acc_reg;

    // =========================================================================
    // INT8 Path (New in Stage 4)
    // =========================================================================
    // INT8: signed 8-bit multiply-add
    // result[15:0] = A_int[7:0] * B_int[7:0] + C_int[15:0]
    wire signed [7:0]  int8_a = A[7:0];
    wire signed [7:0]  int8_b = B[7:0];
    wire signed [15:0] int8_c = C[15:0];
    wire signed [15:0] int8_product = int8_a * int8_b;  // 8x8 signed -> 16bit
    wire signed [16:0] int8_sum = {int8_product[15], int8_product} + {int8_c[15], int8_c};
    wire int8_overflow_pos = (~int8_sum[16] & int8_sum[15]) | (int8_sum[16] & ~int8_sum[15]);
    // Simpler: overflow when carry into MSB differs from carry out
    wire int8_ov = (int8_sum[16] != int8_sum[15]);
    wire [15:0] int8_result = int8_ov ?
        (int8_sum[16] ? 16'h8000 : 16'h7FFF) : // Saturate
        int8_sum[15:0];

    // =========================================================================
    // FP32/FP16 Field Extraction (same as Stage 3)
    // =========================================================================
    ...
    wire c_is_zero = is_acc ? c_eff_is_zero : (use_fp16_fields ? c16_is_zero : ((c32_exp == 0) && (c32_man == 0)));
    ...
    wire        c_sign_fp = is_acc ? c_eff_sign : (use_fp16_fields ? c16_sign : c32_sign);
    wire [23:0] c_mantissa = c32_sel ? c32_eff_mantissa : {c16_mraw, 13'b0};
    wire [7:0]  c_exp_val  = c32_sel ? c32_eff_exp_adj  : c16_exp_fp32;
    ...
    // =========================================================================
    // FP8 Path (same as Stage 3)
    // =========================================================================
    wire [7:0] a_fp8[0:3], b_fp8[0:3], c_fp8[0:3];
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // INT8: compute in Stage 1
            s1_int8_result      <= int8_result;
            s1_int8_ov          <= int8_ov;
            for (i = 0; i < 4; i = i + 1) begin
    ...
    // FP8 alignment
    reg [9:0] fp8_prod_aligned[0:3], fp8_c_aligned[0:3];
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // INT8 passthrough
            s2_int8_result      <= s1_int8_result;
            s2_int8_ov          <= s1_int8_ov;
            for (i = 0; i < 4; i = i + 1) begin
    ...
    generate
        for (gi = 0; gi < 4; gi = gi + 1) begin : fp8_norm
            ...
            assign fp8_result_out[gi] = s2_fp8_nan[gi] ? {fp8_rsign, 4'hF, 3'h7} :
    ...
    wire is_int8_out = (s2_mode == 3'b011);
    wire is_acc_out  = (s2_mode[2] == 1'b1);
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (s2_valid) begin
                if (is_int8_out) begin
                    // INT8 MAD result
                    result   <= {16'b0, s2_int8_result};
                    overflow <= s2_int8_ov;
                end else if (is_fp8_out) begin
                    result <= {fp8_result_out[3], fp8_result_out[2], fp8_result_out[1], fp8_result_out[0]};
